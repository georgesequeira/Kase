...
public class Mark: NSManagedObject {
    ...
    func doesRawImageExist() -> Bool {
        return Mark._doesImageExist(filename: self.rawImageFilename)
    }
    
    func doesAnnotatedImageExist() -> Bool {
        return Mark._doesImageExist(filename: self.annotatedImageFilename)
    }

    func saveOriginalImage(image: UIImage) {
        return Mark._saveImage(image: image, filename: self.rawImageFilename)
    }

    func saveAnnotatedImage(image: UIImage) {
        return Mark._saveImage(image: image, filename: self.annotatedImageFilename)
    }

    func loadOriginalImage() -> UIImage? {
        return Mark._loadImage(filename: self.rawImageFilename)
    }
    
    func loadAnnotatedImage() -> UIImage? {
        return Mark._loadImage(filename: self.annotatedImageFilename)
    }
